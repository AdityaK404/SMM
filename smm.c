...
long now_seconds(void) { return (long)time(NULL); }
...
void ui_register(App *app){
    if (app->graph.user_count >= MAX_USERS){ puts("User limit reached."); return; }
    ...
    printf("Set password: "); if (!get_line(p,sizeof p)) return;
...
void ui_login(App *app){
    ...
    printf("Password: "); if (!get_line(p,sizeof p)) return;
...
void ui_create_post(App *app){
    ...
    if (app->posts.size >= MAX_POSTS){ puts("Post limit reached."); return; }
...
int main(void){
    ...
    for (;;){
        ...
        switch(choice){
            ...
            case 0: app_free(&app); puts("Bye!"); return 0;
